...
class Autorec(nn.Module):
    ...
    def forward(self,torch_input):
        ...
        encoder_output = self.encoder(torch_input)
        output = self.decoder(encoder_output)

        return output

    def loss(self,predict,input,optimizer,mask_input):
        cost = 0
        temp2 = 0

        cost += (( output - input) * mask_input).pow(2).sum()
        rmse = cost
...
def train(epoch):
    ...
    for step, (batch_x, batch_mask_x, batch_y) in enumerate(loader):
        ...
        decoder = rec(batch_x)
        loss, rmse = rec.loss(predict=output, input=batch_x, optimizer=optimer, mask_input=batch_mask_x)
        optimer.zero_grad()
        loss.backward()
        optimer.step()
        cost_all += loss
...
def test(epoch):
    ...
    mse = ((output - test_r_tensor) * test_mask_r_tensor).pow(2).sum()
