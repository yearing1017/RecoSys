...
if __name__ == '__main__':
    ...
    # 召回相关特征
    ## itemcf 相关 将之前的召回的新闻相似度矩阵 拿出来 构建下面两个特征 保存下来
    ...
    df_feature['user_clicked_article_itemcf_sim_sum'] = df_feature[[
        'user_id', 'article_id']].parallel_apply(func_if_sum, axis=1)
    # 待预测新闻和用户最近一次点击新闻相似度
    df_feature['user_last_click_article_itemcf_sim'] = df_feature[[
        'user_id', 'article_id']].parallel_apply(func_if_last, axis=1)
    ...
    df_feature['user_last_click_article_w2v_sim'] = df_feature[[
        'user_id', 'article_id']].parallel_apply(func_w2w_last_sim, axis=1)
    # 待预测新闻和用户所有历史点击新闻相似度按次序加权求和
    df_feature['user_click_article_w2w_sim_sum_2'] = df_feature[[
        'user_id', 'article_id']].parallel_apply(lambda x: func_w2w_sum(x, 2), axis=1)
